<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#nueAlumno').click(function(){
			...
			$('#modalAlumno').find('.modal-title').text('Nuevo Alumno');
				...
				$.ajax({
					...
					success: function(respuesta){
						...
						if(respuesta=="add"){
							...
							alertify.notify('Actualizar exitosamente','success',10,null);
		...
		function validar(){
			...
			if($nombre==0 || $nombre>30){
				...
				$('#nombre').attr('placeholder','Campo Obligario');
			...
			if($apellido==0 || $apellido>30){
				...
				$('#apellido').attr('placeholder','Campo Obligario');
			...
			if($anio==''){
				...
				$('#anio').attr('placeholder','Campo Obligario');
